...
class LayoutTrainer(object):
    def __init__(self, output_dir, dataloader_train, imsize, dataset_test):
        ...
        self.optimizer_bbox = None
        self.criterion_bbox = None

        # Training
        self.imgs_tcpu, self.graph, self.real_box, self.objs_vector, self.key = None, None, None, None, None

        # Evaluation
        self.imgs_tcpu_test, self.graph_test, self.bbox_test, \
            self.objs_vector_test, self.key_test = None, None, None, None, None
    ...
    def prepare_data_test(self, data):
        ...
        # real_vimgs = []
        test_vgraph, test_vbbox, test_vobjs_vector = [], [], []
        if cfg.CUDA:
            for i in range(len(graph)):
                # vgraph.append((graph[i][0].to(self.device), graph[i][1].to(self.device)))
                test_vgraph.append(graph[i].to(self.device))
                test_vbbox.append((bbox[i][0].to(self.device), bbox[i][1].to(self.device)))
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            print(
                '\033[1;31m current_epoch[{}] current_loss[{}] \033[0m \033[1;34m best_epoch[{}] best_loss[{}] \033[0m'.format(
            ...
            if epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:
                ...
                for step_test, data_test in enumerate(self.dataloader_test, 0):
                    # get data
                    self.imgs_tcpu_test, self.graph_test, self.bbox_test, \
                        self.objs_vector_test, self.key_test = self.prepare_data_test(
                        data_test)
                    ...
                    if step_test == 0:
                        save_img_results_test(self.imgs_tcpu_test, self.bbox_test, boxes_pred_test_collection, epoch,
                                              self.image_dir_test)
            ...
            # plot
            plt.figure(0)
            print(self.training_epoch)
            print(self.testing_epoch)
            print(self.training_error)
            plt.plot(self.training_epoch, [error.cpu().detach().numpy() for error in self.training_error], color="r",
                     linestyle="-", linewidth=1, label="training")
            plt.plot(self.testing_epoch, [error.cpu().detach().numpy() for error in self.testing_error], color="b",
            ...
            try:
                print('[%d/%d][%d] Loss_total: %.5f Loss_bbox: %.5f Time: %.2fs' % (epoch, self.max_epoch,
                                                                                    self.num_batches, err_total,
                                                                                    cfg.TRAIN.COEFF.BBOX_LOSS * err_bbox,
                                                                                    end_t - start_t))
            except IOError as e:
                print(e)
                pass
    ...
    def evaluate(self):
        ...
        for step_test, data_test in enumerate(self.dataloader_test, 1):
            ...
            save_img_results_test(self.imgs_tcpu_test, self.bbox_test, boxes_pred_test_collection, step_test,
                                  self.image_dir_eval)
