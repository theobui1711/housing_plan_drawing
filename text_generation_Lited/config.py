...
parser.add_argument('--workers', type=int, help='number of data loading workers',
                    default=0)  # 0 means a single main process
parser.add_argument('--outputFolder', default='.', help='folder to output images and model checkpoints')
parser.add_argument('--trainOverfit', type=bool, default=False,
...
parser.add_argument('--ngf', type=int, default=80,
...
parser.add_argument('--skipConnections', type=bool, default=True,
                    help='skip connections in  Unet -- allows better content reconstruct')
parser.add_argument('--Ubottleneck', type=int, default=-1,
...
parser.add_argument('--fEntropy', type=float, default=.5,
...
def kl_reconstruction_loss(recon_x, x, mu, logvar, opt):
    from util.utils import compute_mean_covariance
...
if opt.outputFolder == '.':
    ...
    opt.outputFolder = "results/" + opt.texturePath[i + 1:] + opt.contentPath[
                                                              i2 + 1:]  # actually 2 nested folders -- cool
