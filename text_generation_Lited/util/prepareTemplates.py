...
def getImage(name, bDel=False):
    ...
    if not bDel:  ##so texture, may rescale
        if opt.textureScale != 1:
            img = img.resize((int(img.size[0] * opt.textureScale), int(img.size[1] * opt.textureScale)),
                             PIL.Image.LANCZOS)
    else:
        if opt.contentScale != 1:
            img = img.resize((int(img.size[0] * opt.contentScale), int(img.size[1] * opt.contentScale)),
                             PIL.Image.LANCZOS)
...
def getTemplates(opt, N, vis=True, path=str(bMirror)):
    ...
    if vis:
        vutils.save_image(out[:8].float(), path + 'templates.jpg', normalize=True, nrow=4,
                          padding=10)  ##limit to 25 the shown templates
