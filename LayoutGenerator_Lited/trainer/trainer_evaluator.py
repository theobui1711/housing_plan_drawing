...
def trans_hull_boxes_coord(point_hull, boxes_type, data_type):
    ...
    for i in range(len(point_hull)):
        if i == len(point_hull) - 1:
            hull_coord.append(
                [float('%.4f' % (float(point_hull[i][0]) / 256)), float('%.4f' % (float(point_hull[i][1]) / 256)),
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.dataloader_train, 0):
                ...
                if step == 299:
                    self.logger.info(
                        "In epoch {}, pred_score1: {}, pred_score2: {}".format(epoch, pred_score1[0], pred_score2[0]))
            ...
            with torch.no_grad():
                ...
                for step, data in enumerate(self.dataloader_test, 0):
                    ...
                    test_layout1 = torch.cat((test_layout1_init_contour, test_layout1_room), dim=1).transpose(0,
                                                                                                              1).cuda()
                    test_layout2 = torch.cat((test_layout2_init_contour, test_layout2_room), dim=1).transpose(0,
                                                                                                              1).cuda()
                    ...
                    test_indicator_values = test_indicator_values[0].reshape(
                        [len(test_indicator_values[0]), 1]).float().cuda()
            ...
            end_t = time.time()
            self.logger.info(
                '[{}/{}] Train_Loss_total: {:.5} Test_Loss_total: {:.5} Time: {:.5}s'.format(epoch, self.max_epoch,
                                                                                             total_err, test_total_err,
                                                                                             end_t - start_t))
        ...
        with open(save_score_path, "w") as score_file:
            for step, data in enumerate(self.dataloader_test, 0):
                ...
                for i in range(save_num):
                    ...
                    eval_layout1_hull, new_contour_type = room_size_range_hull(eval_layout1_hull, contour_types[0],
                                                                               room_classes_size_range)
                    eval_layout2_hull, new_contour_type = room_size_range_hull(eval_layout2_hull, contour_types[0],
                                                                               room_classes_size_range)
                    pred_score = [eval_score1[i][0], eval_score2[i][0]]
                    save_path = os.path.join(self.eval_dir, "layout_score_{}_{}.png".format(num, i))
                    visualization([eval_layout1_hull, eval_layout2_hull], pred_score, save_path, new_contour_type,
                                  "none", draw_text=True)
